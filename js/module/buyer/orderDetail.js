...
OrderDetail.buildStatus1 = function (orderInfoVo, userAccountRecord) {
    var goods = orderInfoVo.orderGoodsVos[0];
    var farmer = goods.farmerId;
    var _html = '<div class="order_line" style="width: 550px;margin: 40px 150px 25px;">' +
        ...
        '<h4>' +
        splitTime(orderInfoVo.createTime) +
        '</h4>' +
        '</li>' +
        '<li>' +
        '<h3>付款</h3>' +
...
OrderDetail.buildStatus2 = function (orderInfoVo, userAccountRecord) {
    ...
    var _html = '<div class="order_line" style="width: 550px;margin: 40px 150px 25px;">' +
        ...
        '<div class="fl details_part_list">' +
        '<h3>付款方式：证联支付</h3>' +
        '<h3>付款时间：' + (userAccountRecord == null || userAccountRecord == "" ? '--' : userAccountRecord.createTime) + '</h3>' +
        '<h3>到账时间：' + (userAccountRecord == null || userAccountRecord == "" ? '--' : userAccountRecord.updateTime) + '</h3>' +
        '<h3>证联交易号：' + (userAccountRecord == null || userAccountRecord == "" ? '--' : userAccountRecord.seqId) + '</h3>' +
        '</div>' +
        '<h1 class="fr details_part_money">￥' +
        '<span style="color: #F76096;font-weight: bold;font-size: 18px;">' + (userAccountRecord == null || userAccountRecord == "" ? '--' : userAccountRecord.amount) + '</span>' +
...
OrderDetail.receive = function (id) {
    ...
    var ajaxObj = {
        ...
        success: function (data) {
            if (data != null && data.code == '1000') {
                layer.alert("已完成收货", function (index) {
